...
<div class="field">
    ...
    <input type="text" class="flatpickr-date" id="{{ $inputId }}" name="{{ $name }}" value="{{ $inputValue }}"
        placeholder="gg/mm/aaaa" autocomplete="off"
        data-alt-class="input @error($name) is-invalid @enderror" @if ($required) required @endif>
    @error($name)
