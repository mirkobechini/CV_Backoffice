...
@section('content')
    ...
    <div class="form-card">
        <form id="tire-edit-form" method="POST" action="{{ route('admin.tires.update', $tire->id) }}"
            ...
            <div class="form-section" style="margin-bottom:0;">
                ...
                <div class="row2">
                    ...
                    <div class="field">
                        ...
                        <select class="select @error('season') is-invalid @enderror" id="season" name="season"
                            required>
                            <option value="" disabled>{{ __('Seleziona una stagionalità') }}</option>
                            <option value="summer" {{ old('season', $tire->season) == 'summer' ? 'selected' : '' }}>
                                {{ __('Estive') }}</option>
                            <option value="winter" {{ old('season', $tire->season) == 'winter' ? 'selected' : '' }}>
                                {{ __('Invernali') }}</option>
                            <option value="all_season"
                                {{ old('season', $tire->season) == 'all_season' ? 'selected' : '' }}>
                ...
                <div class="row2">
                    ...
                    <div class="field">
                        ...
                        <select class="select @error('status') is-invalid @enderror" id="status" name="status"
                            ...
                            <option value="mounted" {{ old('status', $tire->status) == 'mounted' ? 'selected' : '' }}>
                                {{ __('Montate') }}</option>
                            <option value="stored" {{ old('status', $tire->status) == 'stored' ? 'selected' : '' }}>
                                {{ __('In magazzino') }}</option>
                            <option value="retired" {{ old('status', $tire->status) == 'retired' ? 'selected' : '' }}>
                                {{ __('Dismesse') }}</option>
                        </select>
                        @error('status')
                            <div class="field-error">{{ $message }}</div>
                ...
                <div class="row2">
                    <div class="field">
                        ...
                        <input type="text" class="input @error('brand') is-invalid @enderror" id="brand"
                            name="brand" value="{{ old('brand', $tire->brand) }}" placeholder="{{ __('es. Michelin') }}">
                        @error('brand')
                            <div class="field-error">{{ $message }}</div>
                        @enderror
                    </div>
                    <div class="field">
                        ...
                        <input type="text" class="input @error('model_name') is-invalid @enderror" id="model_name"
                            name="model_name" value="{{ old('model_name', $tire->model_name) }}"
                            placeholder="{{ __('es. Alpin 6') }}">
                        @error('model_name')
                            <div class="field-error">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="field">
                    ...
                    <input type="text" class="input @error('size') is-invalid @enderror" id="size" name="size"
                        value="{{ old('size', $tire->size) }}" placeholder="{{ __('es. 205/55 R16') }}">
                    @error('size')
                        <div class="field-error">{{ $message }}</div>
                    @enderror
                </div>
                <div class="row2">
                    ...
                    <div class="field">
                        ...
                        <input type="number" class="input @error('mounted_mileage') is-invalid @enderror"
                            id="mounted_mileage" name="mounted_mileage"
                            value="{{ old('mounted_mileage', $tire->mounted_mileage) }}" min="0">
                ...
                <div class="row2">
                    ...
                    <div class="field">
                        ...
                        <input type="number" class="input @error('next_change_mileage') is-invalid @enderror"
                            ...
                            value="{{ old('next_change_mileage', $tire->next_change_mileage) }}" min="0">
                ...
                <div class="field">
                    ...
                    <textarea class="input @error('notes') is-invalid @enderror" id="notes" name="notes" rows="3">{{ old('notes', $tire->notes) }}</textarea>
                    @error('notes')
                        <div class="field-error">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-actions">
                <button id="tire-edit-submit-btn" type="submit" class="btn primary lg"
